# services/regex_rules.py
from __future__ import annotations
import re
from typing import Callable, Iterator, Match

# ===== 기본 플래그 =====
X = re.VERBOSE
I = re.IGNORECASE
M = re.MULTILINE
S = re.DOTALL

# ---------------------------
# 정규식 패턴
# ---------------------------
PATTERNS = {
    # PHONE (전화번호)
"PHONE": re.compile(r"""
(?x)
(?<!\d)
(
  \+82[ -]?(?:10|11|16|17|18|19|2|[3-6][1-5]|[7-9]\d)[ -]?\d{3,4}[ -]?\d{4}
  |
  0(?:10|11|16|17|18|19)[ -]?\d{3,4}[ -]?\d{4}
  |
  0(?:2|[3-6][1-5]|[7-9]\d)[ -]?\d{3,4}[ -]?\d{4}
  |
  0(?:70|80)[ -]?\d{3,4}[ -]?\d{4}
  |
  1(?:5|6|8)\d{2}[ -]?\d{4}
  |
  \+
  (?=(?:\D*\d){8,15})
  [1-9]\d{0,2}(?:[ -]?\d{2,4}){2,5}
)
(?!\d)
""", re.VERBOSE),

"EMAIL": re.compile(r"""
(?:^|[\s<\(\["':.,;!?=]|[가-힣])     # 앞 경계(소비)
(?:
  # 1) "표시명" + <이메일>
  (?:"[^"\r\n]*"\s*)?<
    ([A-Za-z0-9._%+-]{1,64}@
      (?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)+
      [A-Za-z]{2,63}
    )
  >
  |
  # 2) 일반 이메일 (연속 점 규칙만 체크)
  (?!\.)
  (?!.*\.\.)
  ([A-Za-z0-9._%+-]{1,64}@
    (?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)+
    [A-Za-z]{2,63}
  )
)
(?=$|[\s>\)\]"':.,;!?=]|[가-힣])      # 뒤 경계(lookahead)
""", re.X | re.I),

"PERSONAL_CUSTOMS_ID": re.compile(
    r"(?i)(?<![0-9A-Za-z])P-?\d{12}(?![0-9A-Za-z])"
),

    # RESIDENT_ID (주민등록번호)
    "RESIDENT_ID": re.compile(r"""
(?x)
(?<!\d)
(?: (?:0[1-9]|[1-9]\d)
    (?:
      (?:0[13578]|1[02])(?:0[1-9]|[12]\d|3[01]) |
      (?:0[469]|11)(?:0[1-9]|[12]\d|30)        |
      02(?:0[1-9]|1\d|2\d)
    )
)
-[1-4]\d{6}
(?!\d)
""", X),

"PASSPORT": re.compile(
    r"(?<![0-9A-Za-z])(?:[A-Z]\d{8}|[A-Z]{2}\d{7})(?![0-9A-Za-z])"
),

    # DRIVER_LICENSE
    "DRIVER_LICENSE": re.compile(r"(?<!\d)\d{2}-\d{2}-\d{6}-\d{2}(?!\d)"),

    # FOREIGNER_ID (외국인등록번호)
    "FOREIGNER_ID": re.compile(r"""
(?x)
(?<!\d)
(?: (?:0[1-9]|[1-9]\d)
    (?:
      (?:0[13578]|1[02])(?:0[1-9]|[12]\d|3[01]) |
      (?:0[469]|11)(?:0[1-9]|[12]\d|30)        |
      02(?:0[1-9]|1\d|2\d)
    )
)
-[5-8]\d{6}
(?!\d)
""", X),

    # BUSINESS_ID (사업자등록번호)
    "BUSINESS_ID": re.compile(r"(?<!\d)\d{3}-\d{2}-\d{5}(?!\d)"),

    # MILITARY_ID (군번)
    "MILITARY_ID": re.compile(r"(?<![A-Za-z0-9])\d{2}-(?:\d{5}|\d{8})(?![A-Za-z0-9])"),
    
    # API_KEY (구글/오픈AI/카카오)
    "API_KEY": re.compile(r"""
(?x)
(?<![A-Za-z0-9])
(?:
  AIza[0-9A-Za-z_-]{35}
  |
  (?:sk-[A-Za-z0-9]{32,64} | sk-(?:live|test)-[A-Za-z0-9]{24,64} | sk-proj-[A-Za-z0-9]{24,64})
  |
  (?i:KakaoAK)\s+(?P<kakao>[0-9a-f]{32})
)
(?![A-Za-z0-9])
""", X),

    # GITHUB_PAT
    "GITHUB_PAT": re.compile(r"""
(?xi)
(?<![0-9A-Za-z])
(?:
  (?:ghp|gho|ghu|ghs|ghr)_[A-Za-z0-9_]{36}
  |
  github_pat_[A-Za-z0-9_]{22,255}
)
(?![0-9A-Za-z])
""", X | I),

# PRIVATE_KEY (PEM / OpenSSH / PGP 개인키 블록 감지)
"PRIVATE_KEY": re.compile(
    r"-----BEGIN OPENSSH PRIVATE KEY-----\s*[A-Za-z0-9+/=\r\n]+-----END OPENSSH PRIVATE KEY-----"
    r"|-----BEGIN (?:RSA|EC|DSA) PRIVATE KEY-----\s*[A-Za-z0-9+/=\r\n]+-----END (?:RSA|EC|DSA) PRIVATE KEY-----"
    r"|-----BEGIN (?:ENCRYPTED )?PRIVATE KEY-----\s*[A-Za-z0-9+/=\r\n]+-----END (?:ENCRYPTED )?PRIVATE KEY-----"
    r"|-----BEGIN PGP PRIVATE KEY BLOCK-----\s*[\s\S]+?-----END PGP PRIVATE KEY BLOCK-----",
    M | S,
),


    "CARD_NUMBER": re.compile(r"""
    (?x)
    (?<!\d)
    (?!\d{6}-[1-8]\d{6}(?!\d))   # 6자리-하이픈-7자리 → 주민/외국인번호로 제외
    (?=[\d -]*-)                 # ★ 숫자/공백/하이픈 안에 적어도 한 번 '-'가 포함되어야 함
    (?:\d[ -]?){13,19}           # 숫자 13~19개, 사이에 공백 또는 '-' 허용
    (?!\d)
    """, re.VERBOSE),

    # IMEI (루한은 후처리)
    "IMEI": re.compile(r"(?x)(?<!\d)(?:\d[ -]?){15}(?!\d)"),

"CARD_EXPIRY": re.compile(r"""
(?x)
(?<!\d)                     # 바로 앞에 숫자 오면 안 됨
(0[1-9]|1[0-2])             # 01~12 (월)
\s*\/\s*                    # / 구분자만 허용
(?:
  (?:2[3-9]|3[0-9])         # YY: 23~39 같은 형식 (예: 24, 25, ... 39)
  |
  20(?:2[3-9]|[3-9]\d)      # YYYY: 2023~2099 (예: 2024, 2030, 2099)
)
(?!\d)                      # 바로 뒤에 숫자 오면 안 됨
(?!\s*\d{1,2}:\d{2})        # 뒤에 ' 10:30' 같은 시간 패턴이 오면 제외
""", re.VERBOSE),

# BANK_ACCOUNT (국내 주요 시중/인터넷은행/지방은행/상호금융 계좌번호 – 하이픈/공백 구분형)
# - 앞뒤 숫자 경계: (?<!\d) ... (?!\d)
# - 각 패턴은 "지점코드-중간식별-고객번호" 등 실제 계좌번호 하이픈 위치를 반영

"BANK_ACCOUNT": re.compile(r"""
(?x)
(?<!\d)
(?!\d{4}[- ]\d{4}[- ]\d{4}[- ]\d{4}(?!\d))   # 🔴 4-4-4-4 카드번호는 계좌로 보지 않음
(?:
  # -------------------------------------
  # KB국민은행 / 우체국 (일부 포맷 공유)
  # -------------------------------------
  # 국민 10자리: 000-00-00000 (3-2-5)
  \d{3}[- ]\d{2}[- ]\d{5} |

  # 국민 12자리: 000-00-0000-000 (3-2-4-3)
  \d{3}[- ]\d{2}[- ]\d{4}[- ]\d{3} |

  # 국민(구) / 우체국 14자리: 000000-00-000000 (6-2-6)
  \d{6}[- ]\d{2}[- ]\d{6} |

  # -------------------------------------
  # 우리은행
  # -------------------------------------
  \d{4}[- ]\d{3}[- ]\d{6} |

  # -------------------------------------
  # 하나은행 (KEB 하나)
  # -------------------------------------
  \d{3}[- ]\d{6}[- ]\d{5} |
  \d{3}[- ]\d{3}[- ]\d{6} |

  # -------------------------------------
  # 신한 / 수협 / 카카오뱅크 11자리 계열
  # -------------------------------------
  \d{3}[- ]\d{2}[- ]\d{6} |
  \d{3}[- ]\d{3}[- ]\d{6} |

  # -------------------------------------
  # IBK 기업은행
  # -------------------------------------
  \d{3}[- ]\d{6}[- ]\d{2}[- ]\d{3} |
  \d{3}[- ]\d{3}[- ]\d{7} |

  # -------------------------------------
  # NH 농협 / BNK 부산 / BNK 경남 / 수협 일부
  # -------------------------------------
  \d{3}[- ]\d{4}[- ]\d{4}[- ]\d{2} |
  \d{3}[- ]\d{4}[- ]\d{5} |

  # -------------------------------------
  # 새마을금고 / 산림조합 / 일부 상호금융
  # -------------------------------------
  \d{4}[- ]\d{4}[- ]\d{4}[- ]\d{1} |
  \d{3}[- ]\d{2}[- ]\d{6}[- ]\d{1} |

  # -------------------------------------
  # 인터넷은행 (카카오뱅크 / 케이뱅크 / 토스뱅크)
  # -------------------------------------
  \d{4}[- ]\d{2}[- ]\d{7} |

  # -------------------------------------
  # 전북 / 씨티 / 제주 / 대구 / KDB산업 등
  # -------------------------------------
  \d{3}[- ]\d{6}[- ]\d{3} |
  \d{2}[- ]\d{2}[- ]\d{6} |
  \d{3}[- ]\d{2}[- ]\d{6}[- ]\d{1} |
  \d{3}[- ]\d{4}[- ]\d{4}[- ]\d{3}
)
(?!\d)
""", X),

"HD_WALLET": re.compile(
    r"(?<![0-9A-Za-z])"                       # 앞에 ASCII 영문/숫자가 오면 안 됨
    r"(?:xpub|ypub|zpub|tpub|xprv|yprv|zprv|tprv)"
    r"[1-9A-HJ-NP-Za-km-z]{100,112}"          # 본문(Base58, 길이 제한)
    r"(?![0-9A-Za-z])"                        # 뒤에 ASCII 영문/숫자가 오면 안 됨
),

"PAYMENT_URI_QR": re.compile(r"""
(?x)
(?<![0-9A-Za-z])
(?:
  # -----------------------------
  # 1) Bitcoin URI (BIP-21)
  # -----------------------------
  bitcoin:
    (?:
      [13mn][1-9A-HJ-NP-Za-km-z]{25,34}   # legacy / P2SH / testnet
      |
      bc1[ac-hj-np-z02-9]{11,71}         # bech32 (bc1...)
    )
    (?:\?[A-Za-z0-9%&=_+.\-]+)?
  |
  # -----------------------------
  # 2) Ethereum URI
  # -----------------------------
  ethereum:
    0x[a-fA-F0-9]{40}
    (?:\?[A-Za-z0-9%&=_+.\-]+)?
  |
  # -----------------------------
  # 3) XRP URI
  # -----------------------------
  xrp:
    r[1-9A-HJ-NP-Za-km-z]{24,34}
    (?:\?[A-Za-z0-9%&=_+.\-]+)?
  |
  # -----------------------------
  # 4) Solana URI
  # -----------------------------
  solana:
    [1-9A-HJ-NP-Za-km-z]{32,44}
    (?:\?[A-Za-z0-9%&=_+.\-]+)?
  |
  # -----------------------------
  # 5) Tron URI
  # -----------------------------
  tron:
    T[1-9A-HJ-NP-Za-km-z]{33}
    (?:\?[A-Za-z0-9%&=_+.\-]+)?
  |
  # -----------------------------
  # 6) Bitcoin 생 주소 (URI 없이)
  # -----------------------------
  (?:[13mn][1-9A-HJ-NP-Za-km-z]{25,34}
   |
   bc1[ac-hj-np-z02-9]{11,71})
  |
  # -----------------------------
  # 7) Ethereum 생 주소 (0x...)
  # -----------------------------
  0x[a-fA-F0-9]{40}
)
(?![0-9A-Za-z])
""", X | I),

    # IPV4 (CIDR/포트)
    "IPV4": re.compile(r"""
(?x)
(?<!\d)
(?:
  (?:25[0-5]|2[0-4]\d|1?\d?\d)\.
  (?:25[0-5]|2[0-4]\d|1?\d?\d)\.
  (?:25[0-5]|2[0-4]\d|1?\d?\d)\.
  (?:25[0-5]|2[0-4]\d|1?\d?\d)
)
(?:/(?:3[0-2]|[12]?\d))?
(?::(?:6553[0-5]|655[0-2]\d|65[0-4]\d{2}|6[0-4]\d{3}|[1-5]?\d{1,4}))?
(?!\d)
""", X),

# IPV6 (대괄호/비대괄호, zone-id, CIDR, 포트)
"IPV6": re.compile(r"""
(?<![0-9A-Fa-f:])                 # 앞에 다른 IPv6 조각이 붙어 있지 않도록

(
 (?:
  (?:[0-9A-Fa-f]{1,4}:){7}[0-9A-Fa-f]{1,4} |        # 1:2:3:4:5:6:7:8

  (?:[0-9A-Fa-f]{1,4}:){1,7}: |                     # 1:: 또는 1:2:3:4:5:6:7::

  (?:[0-9A-Fa-f]{1,4}:){1,6}:[0-9A-Fa-f]{1,4} |     # 1::8   /   1:2:3:4:5:6::8

  (?:[0-9A-Fa-f]{1,4}:){1,5}(?::[0-9A-Fa-f]{1,4}){1,2} | # 1::7:8 등

  (?:[0-9A-Fa-f]{1,4}:){1,4}(?::[0-9A-Fa-f]{1,4}){1,3} |

  (?:[0-9A-Fa-f]{1,4}:){1,3}(?::[0-9A-Fa-f]{1,4}){1,4} |

  (?:[0-9A-Fa-f]{1,4}:){1,2}(?::[0-9A-Fa-f]{1,4}){1,5} |

  [0-9A-Fa-f]{1,4}:(?:(?::[0-9A-Fa-f]{1,4}){1,6}) |

  :(?:(?::[0-9A-Fa-f]{1,4}){1,7}|:)                 # ::, ::1, ::1:2:3:4:5:6:7
 )
 (?:/(?:12[0-8]|1[01][0-9]|[1-9]?[0-9]))?           # /0 ~ /128  (옵션)
)
(?![0-9A-Fa-f:])                 # 뒤에 다른 IPv6 조각이 붙어 있지 않도록
""", re.X | re.I),

    # MAC_ADDRESS — (수정) 대문자 포함 검증 lookahead 제거
    "MAC_ADDRESS": re.compile(r"(?xi)(?=[0-9A-F]{2}([-:]))[0-9A-F]{2}(?:\1[0-9A-F]{2}){5}"),
}
